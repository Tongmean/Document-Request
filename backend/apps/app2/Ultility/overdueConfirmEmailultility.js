...
const overdueConfirmEmailultility = async (postTitle, requestData, responseData, user ,payload, requestDateitemData) => {
    ...
    toReciver.push(responseItems.response_by, responseItems.assign_processor_email, responseItems.assign_approver_email, responseItems.email, requestItems.email, payload)
    const mailOptions = {
          ...
          <div style="padding: 30px; background-color: #f9f9f9;">
            ...
            <div style="background-color: #ffffff; border-left: 4px solid #004a99; padding: 15px; margin: 20px 0;">
              ...
              <p style="margin: 5px 0;">
                <strong>OverDue Confirmation:</strong>
              </p>
              <p style="margin: 5px 0;"><strong>วันที่ต้องการ:</strong> ${payload.expected_date}</p>
              <p style="margin: 5px 0;"><strong>ยังต้องการอยู่:</strong> ${payload.isneed}</p>
              <p style="margin: 5px 0;"><strong>Remark:</strong> ${payload.Overdue_remark}</p>
                ...
                <p>
                ${
                  Array.isArray(payload.pathItems) && payload.pathItems.length > 0
                    ? `
                      <p>
                        <strong>File Path:</strong><br />
                        ${
                          payload.pathItems
                            .filter(i => i.path) // remove empty paths
