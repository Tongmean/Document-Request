...
export const fetchDocumentitems = async () => {
    try {
      const response = await apiClient.get('exiting/drawingitemRequestexisting/documentitems'); // Send the GET request to the server
  
      // Check the response's 'success' field to determine if the request was successfuldrawingitemRequestexisting/documentitems
